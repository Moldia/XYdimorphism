...
% all samples, single and dual detection
load('E:\PROOOJECTS\10_XY_dimorphism\Info\sample_dual.mat');
Sample_dual = Sample;
load('E:\PROOOJECTS\10_XY_dimorphism\Info\sample_NLGN4.mat');
Sample = [Sample;Sample_dual];

% width
bandwid = 75;
mkdir('Histograms_atan_NLGN4');
% mkdir('Results\Histograms_atan_NLGN4_2std');
...
for s = 1:length(Sample)
    s
    if strcmp(Sample{s,3},'male')   % skip all female samples
        ...
        for n = 1:2
            ...
            % convolute with guassian filter
...
%         
%         % plot average of randomized repeats
%         hold on;
%         xpoint = 1:length(b4);
...
% %         print(gcf,'-dpng','-r500',['Results\Histograms_atan_NLGN4\' Sample{s} '_atan.png']);
% %         legend({'data','bootstrap','+ 2 std','- 2 std',},'Location','NorthEastOutside');
% %         print(gcf,'-dpng','-r500',['Results\Histograms_atan_NLGN4_2std\' Sample{s} '_atan.png']);
        ...
        % new figure to show difference of integrated pixel
        % frequencies between data and randomized
%         hold off;
%         subplot(2,1,2);
...
% %         print(gcf,'-dpng','-r500',['Results\Histograms_atan_NLGN4\' Sample{s} '_integrated.png']);
%         ylabel('relative frequency');
%         box off
%         legend({'data','bootstrap'},'Location','NorthEastOutside');
%         print(gcf,'-dpng','-r500',['Results\Histograms_atan_NLGN4\' Sample{s} '_new.png']);
    end
end

%% save variables and write to a file
% list = 1:length(Confidence_env);
% Confidence_env = [Confidence_env(mod(list,2)==1,1),Confidence_env(mod(list,2)==0,1),Confidence_env(mod(list,2)==1,2),Confidence_env(mod(list,2)==0,2)];
save('Histograms_atan_NLGN4\NLGN4_ratio.mat','Significant','b4','His4_percent','Confidence_env','Integration');

datawrite = [Sample_male,num2cell([Integration,Confidence_env])];
datawrite = datawrite';
fid = fopen(['histograms_atan_NLGN4\integration_NLGN4_' num2str(thres) '_confidence.csv'],'w');
